...
/*   Updated: 2024/10/12 21:03:18 by aorynbay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	eating(t_philo *philo)
{
	...
	if (!check_if_dead(philo))
		printf("\033[32m%d %d is eating\033[0m", get_time_ms(philo->input->start_time), philo->index);	
	safe_mutex_unlock(&philo->input->printf_mutex);
	c_sleep(philo, philo->input->time_to_eat); // implement
}
...
static void sleeping(t_philo *philo)
{
	safe_mutex_lock(&philo->input->printf_mutex);
	if (!check_if_dead(philo))
		printf("\033[35m%d %d is sleeping\033[0m", get_time_ms(philo->input->start_time), philo->index);	
	safe_mutex_unlock(&philo->input->printf_mutex);
	c_sleep(philo, philo->input->time_to_eat); // implement
}
...
static void thinking(t_philo *philo)
{
	safe_mutex_lock(&philo->input->printf_mutex);
	if (!check_if_dead(philo))
		printf("%d %d is thinking", get_time_ms(philo->input->start_time), philo->index);	
	safe_mutex_unlock(&philo->input->printf_mutex);
}
...
void	*routine(void *arg)
{
	...
	// 	return (one_philo(philo)); // implement
